...
`driftcheck repo`:
```
GIT      ??  no origin/main to compare against | tree clean
...
NOTES    run #1 logged → .driftcheck/repo-history.jsonl
```

`driftcheck docs`:
```
README.md   —   no checkable references found
NOTES     run #1 logged → .driftcheck/docs-history.jsonl
```
