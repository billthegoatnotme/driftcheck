...
NOTES    run #2 logged → .driftcheck/repo-history.jsonl
```

`driftcheck docs`:
```
README.md   —   no checkable references found
NOTES     run #2 | 0→0 references since 2026-09-25
```

### Purpose

A small notes app that works offline first and syncs when a
connection comes back. Built for people who take notes in places with
bad signal.

## Checkpoint Log

### Checkpoint 2 — 2026-09-25

Local storage layer landed. Sync design drafted in
`example-app_offline_sync.md`; conflict handling chosen as
last-write-wins per note, revisit if users complain.

### Checkpoint 1 — 2026-09-25

Project started. Scaffolded the app shell and this spec.
